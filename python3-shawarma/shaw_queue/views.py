...
def order_from_site(request):
    ...
    res = make_order_func(content, None, True, None, False, Servery.objects.filter(pk=6).first(), ServicePoint.objects.filter(pk=1).first(), discount=0, is_preorder=False)
