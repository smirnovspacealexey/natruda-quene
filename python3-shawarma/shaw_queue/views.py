...
def order_from_site(request):
    ...
    res = make_order_func(content, 'delivery', True, '88', False, Servery.objects.filter(pk=7).first(), ServicePoint.objects.filter(pk=1).first(), discount=0, is_preorder=False)
